...
switch sample
    ...
    case 'P30MEMXcreMADMEGFR++L'    % sample name
        img_directory = "path/to/P30MEMXcreMADMEGFR++L";     % path to dataset
        output_directory = fullfile(img_directory, 'numorph_align');
        group = ["brain"];
        channel_num = ["561nm","640nm"];    % channel folder name for hierarchy input
        markers = ["RFP","GFP"];    % marker <--> channel
        position_exp = [];    % empty if dataset contains two-level hierarchy folders
        resolution = [0.65, 0.65, 10];    % voxel size (um)
        ls_width = 50;
